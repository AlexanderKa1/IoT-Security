...
class NRF24:
    ...
    def __init__(self, write_pipe, read_pipe, payload_size = 32):
        self.ce_pin = "P9_15"
        self.irq_pin = "P9_16"
        self.channel = 76
        ...
        #---------important paras----------------------
        self.begin(0, 0, 25, 24)#Set CE and IRQ pins
        self.setRetries(15,15)
        self.setPayloadSize(payload_size)
        self.setChannel(0x60)
        self.setDataRate(NRF24.BR_2MBPS)
        self.setPALevel(NRF24.PA_MAX)
        self.setAutoAck(1)
        self.openWritingPipe(write_pipe)
        self.openReadingPipe(1, read_pipe)
        self.startListening()
        self.stopListening()
        self.printDetails()#----print detail-----
        self.startListening()
    ...
    def available(self, pipe_num=[], irq_wait=False, irq_timeout=None):
        status = self.get_status()
        result = False
    ...
    def read(self, length ,timeout = None):
        # Fetch the payload
        if self.available([], True, timeout) == False:
            return None
        buf_len = self.payload_size
        if not self.dynamic_payloads_enabled:
            data_len = min(self.payload_size, buf_len)
        ...
        txbuffer = [NRF24.R_RX_PAYLOAD] + [NRF24.NOP] * (blank_len + data_len + 1)
        payload = self.spidev.xfer2(txbuffer)
        ret = payload[1:data_len + 1]
        ret = b''.join([i.to_bytes(1,'little') for i in ret])

        self.write_register(NRF24.STATUS, NRF24.RX_DR)
        self.read_register(NRF24.FIFO_STATUS & NRF24.RX_EMPTY)
        return ret
